...
(defn- validate-version-info []
  (u/step (format "Validate Metabase version-info.json at %s" (c/version-info-url))
    (cond
      (c/pre-release-version?)
      ...
      (let [info           (u/step "Fetch version info"
                             (let [{:keys [status body], :as response} (http/get (str "http://" (c/version-info-url)))]
                               (when (>= status 400)
        ...
        (u/announce "Latest version from %s is %s" (c/version-info-url) latest-version)
